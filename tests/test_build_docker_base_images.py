...
def test_build_docker_base_images_step_given_docker_registry_name_should_build_helm_image():
    registry = 'exampleplatformacr.azurecr.io'
    exit_code = subprocess.call(['./platform/steps/build_docker_base_images.sh', registry])

    assert exit_code == 0
    docker_images_output = subprocess.check_output(['docker', 'images', '-q', 'exampleplatformacr.azurecr.io/helm'])
    assert docker_images_output.decode()
